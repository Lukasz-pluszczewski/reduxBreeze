(function (global, factory) {
	...
	(factory((global['redux-breeze'] = {}),global._,global.redux));
}(this, (function (exports,_,redux) { 'use strict';
 ...
 */
var immutablyCopyValue = function immutablyCopyValue(fromObject, fromPath, toObject, toPath) {
  ...
  return {
      ...
      default: function _default(actionDefinition, actionName, initialState) {
        return function () {
          ...
          if (action.type === createActionType(actionName)) {
            var resultsAssignments = void 0;

            // result is array of assignments
            if (Array.isArray(actionDefinition.result)) {
              resultsAssignments = actionDefinition.result.reduce(function (accu, _ref2) {
                var sourcePath = _ref2.sourcePath,
                    ...
                    value = _ref2$value === undefined ? new hasNotBeenDefined() : _ref2$value;

                // setting default value if one has not been provided and defaultValue is defined
                if (!(defaultValue instanceof hasNotBeenDefined) && _.isUndefined(_.get(action, sourcePath)) && value instanceof hasNotBeenDefined) {
              ...
              var resultName = actionDefinition.resultName || actionName;

              // setting default value if one has not been provided and defaultValue is defined
              if (_.has(actionDefinition, 'defaultValue') && _.isUndefined(action.payload) && _.has()) {
                resultsAssignments = defineProperty({}, resultName, actionDefinition.defaultValue);
              } else if (!_.has(actionDefinition, 'value')) {
                resultsAssignments = defineProperty({}, resultName, action.payload);

                // setting value calculated by a 'value' function
              } else if (_.isFunction(actionDefinition.value)) {
                resultsAssignments = defineProperty({}, resultName, actionDefinition.value(action, state[resultName]));

                // setting hardcoded value from 'value' field in the definition
              } else {
                resultsAssignments = defineProperty({}, resultName, actionDefinition.value);
              }
            }
    ...
    initialStateAdapter: {
      default: function _default(actionDefinition, actionName) {
        ...
        if (Array.isArray(actionDefinition.result)) {
          resultsAssignements = actionDefinition.result.reduce(function (accu, _ref3) {
          ...
          resultsAssignements = defineProperty({}, actionDefinition.resultName, defaultValue);
        }
...
var tools = {
  ...
  immutablyCopyValue: immutablyCopyValue
};
...
exports.tools = tools;
exports['default'] = createReduxBreezeInstance;
